...
class PatientLoginTest(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='patient', password='12patient12', email='test@example.com')
        self.user.save()
    ...
    def test_login_pass(self):
        response = self.client.post('/login', {'username': 'patient', 'password': '12patient12', 'role':'patient'})
        expected_response = status.HTTP_200_OK
        self.assertEqual(response.status_code, expected_response)

    def test_wrong_username(self):
        response = self.client.post('/login', {'username': 'wrong', 'password': '12patient12', 'role':'patient'})
        expected_response = status.HTTP_403_FORBIDDEN
        self.assertEqual(response.status_code, expected_response)

    def test_wrong_password(self):
        response = self.client.post('/login', {'username': 'patient', 'password': 'wrong', 'role':'patient'})
        expected_response = status.HTTP_403_FORBIDDEN
        self.assertEqual(response.status_code, expected_response)

    def test_wrong_role(self):
        response = self.client.post('/login', {'username': 'patient', 'password': '12patient12', 'role':'doctor'})
        expected_response = status.HTTP_403_FORBIDDEN
        self.assertEqual(response.status_code, expected_response)

class DoctorLoginTest(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='doctor', password='12doctor12', email='test@example.com')
        self.user.save()
    ...
    def test_login_pass(self):
        response = self.client.post('/login', {'username': 'doctor', 'password': '12doctor12', 'role':'doctor'})
        expected_response = status.HTTP_200_OK
        self.assertEqual(response.status_code, expected_response)

    def test_wrong_username(self):
        response = self.client.post('/login', {'username': 'wrong', 'password': '12patient12', 'role':'doctor'})
        expected_response = status.HTTP_403_FORBIDDEN
        self.assertEqual(response.status_code, expected_response)

    def test_wrong_password(self):
        response = self.client.post('/login', {'username': 'doctor', 'password': 'wrong', 'role':'doctor'})
        expected_response = status.HTTP_403_FORBIDDEN
        self.assertEqual(response.status_code, expected_response)

    def test_wrong_role(self):
        response = self.client.post('/login', {'username': 'doctor', 'password': '12doctor12', 'role':'patient'})
        expected_response = status.HTTP_403_FORBIDDEN
        self.assertEqual(response.status_code, expected_response)
...
class DoctorTest(APITestCase):
    def setUp(self):
        ...
        doctor = User.objects.create_user(username='doctor', password='doctor', email='doctor@example.com')
        patient = User.objects.create_user(username='patient', password='patient', email='patient@example.com')
        
        doctor.save()
        patient.save()

        token = UserToken.objects.create(user=doctor, verified=True)
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + token.key)

        self.user = Doctor.objects.create(user=doctor)
        self.patient = Patient.objects.create(user=patient)

        HealthRecord.objects.create(user=self.patient,dateofbirth='1998-03-03',height=170,weight=60,bloodtype='A+',allergies='None')
    ...
    def test_view_patient_records(self):
        ...
        serialized_record = PatientRecordsSerializer(HealthRecord.objects.get(user=self.patient))
        serialized_exams = PatientPastSessionSerializer(Examination.objects.all().filter(patient=self.patient), many=True)
        expected_data = {}
